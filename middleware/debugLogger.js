module.exports = (req, res, next) => {
  ...
  // Skip all debug logging for LinkedIn session sync endpoint
  if (req.originalUrl.includes('/linkedin/sync-session')) {
    return next();
  }
  ...
  // Skip logging request body for LinkedIn session sync to avoid cookie spam
  if (!req.originalUrl.includes('/linkedin/sync-session')) {
